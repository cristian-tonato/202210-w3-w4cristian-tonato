...
export class SeriesList extends Component {
  ...
  constructor(public selector: string, watched: boolean) {
    super();
    this.series = new Store().getSeries();
      ...
      html += `<li class="serie">
            ...
            <ul class="score">
              <li class="score__star">
                <i data-name="${
                  element.name
                }" data-star="1" data-name="" class="icon--score ${
        element.score > 0 ? 'fas' : 'far'
      } fa-star" title="1/5"></i>
              </li>
              <li class="score__star">
                <i data-name="${
                  element.name
                }" data-star="2" class="icon--score ${
        element.score > 1 ? 'fas' : 'far'
      } fa-star" title="2/5"></i>
              </li>
              <li class="score__star">
                <i data-name="${
                  element.name
                }" data-star="3" class="icon--score ${
        element.score > 2 ? 'fas' : 'far'
      } fa-star" title="3/5"></i>
              </li>
              <li class="score__star">
                <i data-name="${
                  element.name
                }" data-star="4" class="icon--score ${
        element.score > 3 ? 'fas' : 'far'
      } fa-star" title="4/5"></i>
              </li>
              <li class="score__star">
                <i data-name="${
                  element.name
                }" data-star="5" class="icon--score ${
        element.score > 4 ? 'fas' : 'far'
      } fa-star" title="5/5"></i>
              </li>
            </ul>
            <i data-name="${
              element.name
            }" class="fas fa-times-circle icon--delete"></i>
          </li>`;
    });
  ...
  private scoreStar() {
    document.querySelectorAll('.score__star i').forEach((item) => {
  ...
  private handlerButton(ev: Event) {
    ...
    if (item !== undefined && starTitle !== undefined) {
      ...
      item.watched = true;
      new Store().setSeries(this.series);
